...
<h2>September 9, 2026: more natural companion movement</h2>
<p>Kiana and Della have more varied blinking, subtle brow movement during pauses, and smoother head movement on the website, Android <strong>2.12 (build 113)</strong>, and iPhone TestFlight <strong>282</strong>. Voice playback and animation controls work as before. The public App Store update is still awaiting Apple review.</p>
<h2>September 9, 2026: expressive companions and account emails</h2>
<p>Kiana and Della now have individual brow expressions and movement rhythms on the website, Android <strong>2.11 (build 112)</strong> and iPhone TestFlight <strong>281</strong>. iPhone voice calls can also show the originating companion while listening, thinking and speaking. Version <strong>2.0.2</strong> has been submitted for public App Store review; Apple must approve it before it reaches the store. Pause, reduced motion and the animation switch still apply. When Kade approves access, account information is emailed to the account&rsquo;s email address. Phone-only accounts still receive their sign-in details directly from Kade.</p>
<h2>September 9, 2026: Della and smoother voice portraits</h2>
<p>Della now has her own portrait. On the website and Android <strong>2.10 (build 111)</strong>, Della and Kiana have graduated mouth and eyelid movement during voice messages. Other companions keep their own pictures. This animation adds no voice-generation charge. The corresponding iPhone update is now in TestFlight build 281 and has been submitted for App Store review.</p>
...
<h2>September 11, 2026: the Parlor has a lobby, and Lyria briefs in Google's order</h2>
<p><strong>The Parlor</strong> on the website now shows <em>Open tables right now</em> right under the games: any table a family member dealt with a free seat for friends, with the host, the game, the seats open and the code. Tap it and you are seated &mdash; nobody has to read you a code. The page runs games first, then the lobby, then your own open tables, then the code box, then the standings. Dealing a table with open seats offers a <em>Keep it off the lobby</em> switch. The iPhone app gets the same layout in its next build. In the <strong>Sound Booth</strong>, Lyria's script desk and how-to notes now follow Google's own prompt guide (genre and era, instruments, section tags, the voice, the mood, then BPM, key and length), your own lyrics ride under a proper <em>Lyrics:</em> heading, and the words it writes come back without its machine markers, so the read-back is just the song. See <a href="/help/games">The Game Parlor</a> and <a href="/help/audio">Sound &amp; Audio</a>.</p>
...
<ul>
  ...
  <li><strong>Projects keep working context.</strong> Open a project's <em>Instructions and working files</em> on the web. Add instructions and text references, then let agents revise a working document in later project chats. Import .txt, .md, .csv or .json files, keep earlier versions, and download the finished file. Word and PDF editing are not part of this project-file workflow yet.</li>
</ul>
...
<ul>
  ...
  <li><strong>Clearer narration updates.</strong> Studio agents report the actual wait for your narration job. A missing job is not replaced with another job's status, and a failed render is no longer described as proof that no provider charge occurred.</li>
  ...
  <li><strong>Android 2.8: project files on your phone.</strong> Import UTF-8 text into a draft for review, and export current files, older versions or unsaved drafts through Android's file picker. <a href="/help/android">Download the update</a>.</li>
...
<ul>
  <li><strong>Four things your characters could not actually do, fixed.</strong> "Where am I?", "how do I turn on voice?", "transcribe my voice memo," and the deep-research desk had quietly stopped reaching the tools behind them &mdash; the characters were answering from memory and sounding sure about it. Found while tightening the tools (below), fixed the same hour. If a character told you something about this site that turned out wrong in the last few weeks, that is probably why; ask again.</li>
  <li><strong>Characters carry only the tools a turn needs.</strong> Every reply used to ship the instructions for two dozen tools whether you wanted the phone or were just saying good night. Now a small core rides every turn (memory, reminders, weather, help, feedback) and the rest attach when your words call for them &mdash; "call my mom," "what's in the news," "look that up" &mdash; and stay attached for the rest of that conversation. Replies got a little faster, and it costs the server less. If a character ever seems not to know it has a tool, say the thing plainly ("use the weather tool") and tell Kade.</li>
  <li><strong>What you actually cost the server, on the iPhone (build 271).</strong> Under Settings &rarr; Usage &amp; Balance there is a new line: your real month-to-date cost, beside the balance, with the note that balances are charged at twice real model cost to help cover the rest of the platform. The website's Feed the Server page has had it since last night; Android shows the same page inside the app.</li>
  <li><strong>New characters are born on the family's model.</strong> The Build Your Own Character screen defaulted to the expensive brain; it now starts on the same model every character runs, and one listing that never actually worked is gone from the picker.</li>
</ul>

<h2>September 4, 2026</h2>
<ul>
  <li><strong>Forty-one kids' and teens' voices are filed where they belong.</strong> A friend said some voices were in the wrong section, and she was right: the machine ear that described the catalog listened to every voice reading the same grown-up paragraph, so a child's voice reading it came out labelled "young woman." Kade's own names for her clones settle it &mdash; <em>trevor male kid</em>, <em>scarla female child</em>, <em>megan female teen</em> &mdash; so thirty-five children and teenagers moved out of the women's and men's headings into <strong>Kids and teens</strong> (kids first, then teens, and the labels now say <em>girl</em>, <em>boy</em>, <em>kid</em>, <em>teen girl</em>, <em>teen boy</em>), and three grown voices whose maker's own description said otherwise swapped headings. Nothing you picked moved: a saved voice keeps speaking and quietly takes its new label.</li>
  <li><strong>"Wrong section?" is a button now.</strong> Under any voice in the picker &mdash; web, the agent builder, and the iPhone on its next build &mdash; tap <em>Wrong section?</em>, say what it actually sounds like (a woman, a man, a kid or a teen, a character, older, younger), and it lands on Kade's feedback board with the voice's name and the heading it sits under. Your ear beats the machine's; this is how the next catalog pass gets receipts.</li>
  <li><strong>Your companions can share what you tell them &mdash; facts, not opinions.</strong> If you talk to more than one character and are tired of saying things twice, there is a <strong>Memory sharing</strong> setting (Android now; iPhone on its next build; web soon): off, all of them, or a chosen few. When it is on, a fact you told Della can surface for Kiana marked as secondhand &mdash; she knows the thing without pretending she was there &mdash; and the same the other way. What never crosses over is each character's own read of you: their opinions, their running thread, what they have learned. That is what makes Della Della. Off is truly off; nothing is copied.</li>
  <li><strong>Android gets notifications now (build 2.2).</strong> Reminders, a companion texting you first, calls, and notes like this one reach an Android phone the same way they reach an iPhone. Install the new build from the site (the app offers it when you open it), say yes when it asks to send notifications, and you are on the list. Two channels in Android's settings if you want them different: <em>Kade-AI</em> for the everyday notes and <em>Calls from companions</em>, louder.</li>
  <li><strong>Android has a memory manager and tells you when a new build is up.</strong> Memories, Logbook and Sharing in three tabs, with search, edit and forget. And from this build on, the app checks the site when it opens and offers the download when a newer one is there &mdash; sideloaded apps never did that on their own, so anyone on the August build has to download once more by hand, and after that it takes care of itself.</li>
  <li><strong>Hear the activity logs in their voice, on web.</strong> Every line on the activity-logs page has <em>Hear it in their voice</em> and <em>Play from here</em>, using the voice that person chose for the character; your own lines play in the browser's plain voice so the two are never confused.</li>
</ul>

<h2>September 2, 2026 — evening</h2>
<ul>
  <li><strong>The iPhone voice picker is wheels now (build 264, submitted to App Review), Kade's own design.</strong> Pick <strong>who</strong> first (Women, Men, Kids and teens, Characters), then spin the <strong>voice</strong> wheel &mdash; each row is just the voice's name (Flurry, Cleat, Hickory), and every voice you land on plays its full audition; move on whenever you have heard enough. Hit Done and that is your voice; Cancel leaves it alone. "Use the character's own voice" is a switch at the top. New under the wheel: a <strong>Delivery</strong> control &mdash; Steady, Balanced, or Lively &mdash; saved for that character on your phone and used on every reply and audition (Lively has the most emotional range). Your speed setting now changes how the voice is <em>made</em> instead of stretching the recording afterwards, so 1.25x sounds like the same person talking a little faster, not a sped-up tape. The search box still finds voices by words like <em>husky</em> or <em>British</em>, and an old pick like "Voice 69" opens the wheel on the voice it became.</li>
  <li><strong>The voices have descriptions now, not numbers.</strong> Every one of the 621 voices was listened to fresh on the same plain script and described as an instrument: <em>husky low middle-aged woman, Black American</em>, <em>bright high young woman</em>, <em>gravelly low older man, Southern US</em>. Each description ends with one plain tag word (a tree, a river, a stone, a bit of weather &mdash; never anybody's name) so two voices that sound alike still have their own handle: <em>husky low woman &middot; guppy</em>. Kade Murdock (Fish Audio) and Miss-A (Inworld professional clone) are available under <strong>Women, named voices</strong>. The picker now asks for a <strong>category first</strong> (Women, bright and high, young &mdash; Men, seasoned and gravelly &mdash; Kids and teens &mdash; twenty-three in all), then walks that category's voices, each speaking as you land on it, and the search box understands words like <em>storyteller</em> or <em>Southern</em> because it reads the descriptions too. Your saved voice did not move: an old pick like "Voice 69" quietly becomes its described label the next time you open the picker, and saying "switch to voice 69" on a call still works. Four new older-woman voices joined the same evening.</li>
  <li><strong>137 characters got voices that fit them.</strong> With every voice described by ear, each of the 219 marketplace characters was matched against what its own persona calls for (a woman or a man, young or seasoned, husky or bright, Southern or British, plain or theatrical). Sixty-two were already well cast and kept their voice; the rest moved to a closer fit, and a few outright mismatches are gone (a woman doctor no longer speaks in a man's voice; a grown character is off the cartoon-kid voice). If a favorite character sounds different today, that is why, and if you liked the old one better, tell Kade the name and it goes back in a minute.</li>
  <li><strong>A listening lab for the voices, and one quiet fix.</strong> The tone shaping every voice passes through now runs before the volume is measured instead of after, so the volume stage sees the sound you actually hear, and a bass or treble boost can no longer push a loud voice past full scale (nothing you would have noticed as more than a rare tick, now gone). Then the new part: the <a href="/lab/limiter">listening lab</a> plays the same words on the same voice through four versions of the last stage, each changing one thing, so you can tell Kade on your own speaker where a buzz lives before anything changes for everyone.</li>
...
<ul>
  ...
  <li><strong>The side panel is gone.</strong> Not hidden &mdash; gone. Everything it used to hold is a full page of its own now: <a href="https://kademurdock.com/agent-builder">Agent Builder</a>, <a href="https://kademurdock.com/bookmarks">Bookmarks</a>, <a href="https://kademurdock.com/memories">Memories</a>, <a href="https://kademurdock.com/files">Files</a>, <a href="https://kademurdock.com/settings">Settings</a>, and <a href="https://kademurdock.com/conversations">Your conversations</a>. Each has a heading, a way back Home, and nothing else on the page competing for attention. On a computer the chat screen has one thin strip across the top: Kade Home, New chat, Your conversations, Agent Builder, Marketplace, Settings, Help. On a phone the Chats tab opens the conversation list as a page.</li>
...
<p>Voice replies can show the speaking character while you listen, including outside calls. This is available on the website, in iPhone TestFlight build <strong>281</strong>, and in Android <strong>2.11 (build 112)</strong>. The App Store release may be older.</p>
<p>On the website, Android 2.11 (build 112) and iPhone TestFlight 281, Kiana and Della have their own prepared eyes, mouths and brow expressions, with graduated movement following playback and individual movement rhythms. Della has silver curls and a plum cardigan. On iPhone, voice calls also show the originating companion while listening, thinking and speaking. Mouth movement follows the call&rsquo;s actual output audio. Live camera mode keeps its existing display. Other characters use their own portrait with gentle movement while individual facial artwork is prepared. An unavailable or changed portrait falls back safely. Animation does not change the voice and adds no speech-generation request. Playing an older text reply can still generate speech at the normal voice cost.</p>
...
<p>In the voice picker's Delivery control, <strong>Lively</strong> allows a wider performance range, while <strong>Balanced</strong> and <strong>Steady</strong> reduce variation. Speaking speed is a separate choice. Routine directions such as “unhurried” or “quick” are filtered to help respect that speed; feelings and laughter stay available in every mode. These adjustments apply to Inworld and Fish Audio when you generate speech, including a fresh replay of an older reply. Audio already downloaded or cached keeps its original performance.</p>
...
<p>New replies are guided to change feeling when the conversation calls for it, rather than switching between quiet and upbeat on a schedule. <strong>Lively is the default for Inworld voices.</strong> Your saved Steady, Balanced or Lively choice still wins; Fish keeps its expressive default. Long passages now keep their active feeling when split for speech, and voice auditions start fresh instead of borrowing recent chat context. The new iPhone update also keeps a direction change made partway through a streamed piece. Delivery varies by voice; use Preview to hear your choice. Existing recordings keep the performance they were made with.</p>
...
<p>From September 11 the picker moves a reported voice on its own: when you press <em>Wrong section?</em> and say what it sounds like, the voice is re-filed under that heading within a few minutes, the report is closed with a note, and you hear about it in chat. A voice you already picked keeps working under its old name.</p>
...
<p>For a video with spoken descriptions mixed into its soundtrack, open <a href="https://kademurdock.com/described-video">Make a described video</a>. Adult accounts can upload a video, paste a YouTube link, or start from a Library video they can access. Choose a narrator, description detail and speed. Your default narrator, your favourite voices and the speeds you pick (the usual narration speed, the fastest it may go to fit a gap, and the playback speed) are remembered for your account, the same on the website and the iPhone. Try a preview, describe the whole video, or select a part. Standard mode fits narration into gaps; pause mode can briefly pause the picture and soundtrack for a description.</p>
...
<h3>Reverie is on the menu (September 11, 2026)</h3>
<p>The first tile in the Parlor is not a card game: <strong>Reverie</strong>, the city, opens from the Parlor now, and every family account can walk in. It is the same <a href="https://kademurdock.com/world">kademurdock.com/world</a> page; <em>Back</em> brings you to Kade-AI and the Parlor is one link away. Everything about the city is on its own page, <a href="/help/world">Reverie &mdash; the city</a>.</p>
<h3>The lobby (September 11, 2026)</h3>
<p>Right under the games, the Parlor now shows <strong>Open tables right now</strong>: every table somebody in the family has dealt with a free seat for friends, with the host's name, the game, how many seats are open, and the code. Tap one and you are seated &mdash; nobody has to read you a code any more. The page is laid out games first, then the lobby, then your own open tables, then the code box, then the standings, so a screen reader reaches a game before anything else. When you deal a table with open seats there is a <em>Keep it off the lobby</em> switch; turn it on and your friends need the four-character code from you, the way it always worked. A table drops out of the lobby when it fills, when it ends, or after a day. The iPhone app gets the same layout in its next build.</p>
...
<p>Reverie is open to everyone in the family now. It sits at the top of <a href="https://kademurdock.com/parlor">the Parlor</a> as the first tile, and it is still at <a href="https://kademurdock.com/world">kademurdock.com/world</a> if you would rather go straight there. In the iPhone app the World screen is where it always was. There is no code to enter and nothing to set up: the first time in, the city asks you nine short questions and you are somebody.</p>
...
<p>On the website, <strong>World shortcuts</strong> jump straight to your command, movement, character status, or the world log. Room descriptions appear in full. If you scroll back to read, new arrivals leave your place alone; <strong>Jump to latest</strong> brings you back. Buttons keep their place when the room updates. Your needs have written values as well as bars, and the decorative city picture stays out of the screen reader.</p>
...
<h2>The Band, on the air (September 11, 2026)</h2>
<p>Reverie has one radio station, the Band, broadcasting from over Hock&rsquo;s pawnshop in Tanglefoot, and it is a real station now: short blocks written from what the city was doing the day they were recorded &mdash; the weather, who was hosting a cookout, who held the bowling board, what people were saying at Levi&rsquo;s &mdash; and performed as recordings with voices, room tone and sound effects. The schedule follows the city&rsquo;s clock: <strong>The Morning Tide</strong> with Harlan Pike (tide and weather, five to eleven), <strong>Small Crimes and the Classifieds</strong> with Junie Baptiste (the classifieds read deadpan and a true-crime hour about something trivial, eleven to five), <strong>The Evening Chart</strong> with Harlan again (what is open, the chart argued about, a dedication with no name on it, five to eleven), and <strong>The Overnight Block</strong> with Lorraine Vance (slow, kind, takes requests, eleven at night to five in the morning). Citizens phone in. Sometimes a person out in the city gets a shout-out by first name.</p>
<p>Say or tap <strong>Turn on the radio</strong> anywhere a radio is on: the Band&rsquo;s own studio, Dez&rsquo;s, Pat&rsquo;s, the Truck Stop, the Garages, any car, or a home with a radio from Hock&rsquo;s. On the website the block plays as sound &mdash; the ambience drops under it and an <strong>On the radio</strong> box appears with <em>Play the radio</em>, <em>Turn the radio down</em> and <em>Read the words</em>. If the block does not start on its own, tap <em>Play the radio</em> once; phones want a tap before they will play sound. <strong>Read the words</strong> (or typing <code>radio words</code>) puts the whole transcript in the world log so a screen reader can read it at your pace, and <code>radio off</code> turns it down. On the iPhone app and through a character in chat, the radio arrives as the words themselves. The Band plays from a <strong>shelf</strong> of recorded blocks: each part of the day has its own recordings and the station picks a different one each day, so nothing is made on its own and nothing costs money while you listen. New recordings are made only when Kade asks the Band for one. <code>radio bank</code> says what is on the shelf. When a new recording lands, anybody sitting where a radio is on hears it start.</p>

<h2>Hearing the room without being interrupted (September 11, 2026)</h2>
<p>On the website, everything the city says in one turn now arrives as one piece &mdash; the room, the exits, who is here &mdash; instead of a run of separate announcements a screen reader could start and drop. Things that happen around you while you are being read to (somebody walking in, the freight, a citizen talking) wait for a quiet moment and then come as one piece of their own. A button you are focused on is never pulled away when the room changes; it stays put until you move on. Sound cues are spaced out, never more than four in a turn, and the background ambience dips under them and under the radio. The city also has a day now: gulls at first light on the water, the Bell ringing the wrong hour at eight, half past twelve and six (close in Bellward, far off everywhere else), the noon whistle on the docks and in the Millrace yards, the ferry horn at half past five, the Band signing on at eight in the evening wherever a radio is on, and the taco window&rsquo;s three a.m. line in Tanglefoot. Citizens speak up more often, in every ward where someone is.</p>
...
<h2>The river walk, pictures, and your look</h2>
<p><strong>Find a place</strong> opens neighborhood buttons, then destinations. <strong>River walk</strong> leads to Ropewalk, the Net Loft, and the Reed Pavilion: a connected route from the Hook ferry landing to Sweetwater Park. Each place has its own sounds and things to do together. The loft and pavilion offer shelter.</p>
<p>On the website and Android World, tap a person in the picture to open their interaction menu, or a direction sign to travel. Tap your own figure to dress up. Ordinary labeled buttons offer the same actions with the picture off. Dragging across the picture scrolls the page. Choose <strong>Describe the picture</strong> to hear what it shows. Motion can be stopped.</p>
<p><strong>Dress up</strong> or <code>wardrobe</code> offers hair and clothes. Tap a choice, type <code>hair pink curls</code> or <code>wear a blue dress and headphones</code>, or use <strong>Talk</strong> to dictate it. <code>pink hair</code> works too. Changes are free and keep your possessions. Common hair and outfit colors appear on your figure; custom descriptions remain readable even where their details are simplified in the picture.</p>
<p><strong>Forecast</strong> or <code>forecast</code> describes the sky now and in three, six, and twelve hours. Weather fronts develop gradually, with seasonal temperatures. Rain ambience follows wet weather and becomes quieter indoors. Sound starts after an interaction; Sound &amp; settings controls volume and silence. These commands and destination choices also work in the iPhone app; the new interactive pictures and layered rain currently use the web World, including Safari.</p>
<h2>Finding your way and meeting people</h2>
<p>On the website, Android World and iPhone Safari, <strong>Explore from here</strong> lists the places connected to your current room. Choose a named exit to travel; the picture, people and ambience follow the server-confirmed location. Direction signs in the 3D view show those same exits. The signs are compass markers, not measured distances. Travel is between rooms; free walking within a continuous 3D landscape is still being developed.</p>
<p><strong>Get oriented</strong>, or type <code>orient</code>, reads your place, everyone here, exits and your last room when it has a direct way back. This command also works in the native iPhone app. The way back can become locked, and returning checks the current access rules. Locked exits stay named so you can investigate them. Nobody in an adjacent room is listed here.</p>
<p><strong>Say something to the room</strong> prepares <code>say </code> in the command box. Your words reach everyone present. Select a person, then <strong>Whisper to</strong>, for private words; use <code>whisper "Full Name" your words</code> when typing. A first name works only when it identifies one person here. A draft already in the box is kept. Arrivals and departures update the people list and picture, and reconnecting refreshes who is present. Replies and live messages share the World log. If your location changed before an action arrived, the page shows your current room and asks you to choose again.</p>
<h2>A world you can see, hear, and read</h2>
<p>The website, Android’s World page, iPhone Safari and the native iPhone app in TestFlight 282 now have a three-dimensional miniature view. Walk from a cutaway diner to wooded paths, a creek, camp, or the waterfront. Trees sway, water ripples, lamps light the night, and rain or snow follows the weather. Home furniture and people come from your actual room. Saved build, hair, and clothing choices now shape your figure and other created residents: locs, braids, curls, buns, hats, coats, dresses, and headphones appear in the miniature. Custom clothing is simplified. Colors and unchosen details remain artistic. Describe the picture also reads the chosen appearance. Residents without saved choices retain stylized stand-ins. People ease into their places when they arrive, and gathering guests face the shared table. World motion stops these movements.</p><p>Dez&rsquo;s, the Millrace Lanes and the Records Office now have their own miniature interiors. Residents can hold books, cups, cooking tools and other small objects based on their visible activities. Gathering gestures take precedence while someone joins the group. These gestures illustrate the room; use the text for what actually happens. The Ferry Dock has a quiet engine bed, and courthouse sweeping has broom strokes. Scene audio follows your existing sound controls.</p>
<p>Under <strong>Sound &amp; settings</strong>, <strong>Describe the picture</strong> adds a spoken-readable description to the world log. It describes the artwork, including its colors and composition. <strong>Room picture</strong> turns graphics off; <strong>World motion</strong> stops animation. Your device’s Reduce Motion setting also stops animation. Pictures do not change sound or the available actions. An illustrated fallback is available when 3D cannot run. <strong>Picture controls</strong> turns or zooms the camera; use the room’s exits to travel.</p>
<p>All game details remain in text and buttons. Graphics are hidden from screen readers, and animation pauses when the picture is offscreen or the page is hidden. <strong>Roomy text</strong> gives larger, more widely spaced reading. The native iPhone picture is prepared for a future app update; these web changes do not install it into an older native build.</p>
<h2>The Gully Washhouse</h2>
<p>Go northeast from Gully Road, or type <code>go to Gully Washhouse</code>. Nell Calder keeps the folding table moving, trades mysteries with Ines, and looks after Pat&rsquo;s towels. Choose <strong>Wash clothes</strong> to improve comfort or <strong>Fold laundry</strong> to settle in. There is no coin charge. Wait fifteen seconds between loads.</p>
<p><strong>Sort buttons</strong> lets you work with Nell&rsquo;s blue button tin. It shares the fifteen-second pause with laundry. Open <strong>Window bench</strong> to help with three shared repairs: tighten its leg, sand its edge, and fit felt pads. Each finished repair is saved for everyone. Supplies are provided; there is no money charge.</p>
<p><strong>Book exchange</strong> offers three short mysteries, each in three parts. Choose when to read the next part. Your place in each story is saved for your character, so leaving and returning does not lose it. The books stay at the Washhouse. If a reply was missed, choose Reopen part to hear it again; Previous part lets you go back without changing your bookmark. After an ending, choose Read again to start over. These actions arrive through the shared game service on web and phones. The updated linoleum, blue tin, bench and book shelf are visible in the web picture and Android World; the bundled iPhone picture needs a later app update.</p>
<p>Founder control <code>@resident pilot status</code> reports the existing trial&rsquo;s request reservations and latest saved plans with emitted activity steps. Those snapshots are not a lifetime count or a billing receipt. Starting the trial again cannot refill or extend its allowance.</p>
<p>The washhouse has a quiet recording of real washing machines. Bowling strikes have a recorded ball-and-pins impact. These follow your existing ambience and sound controls.</p>
<p>Levi&rsquo;s Chairs and the Garages now have their own miniature interiors. Figures blink independently, look around, raise their brows, and turn as they walk into position. They can wear the glasses, aprons and watches described in their clothing. These small figures illustrate the written activities; the world log carries the actual events.</p>
<h2>Invite someone into the gathering</h2>
<p>Start or join a cookout, record night, or story circle. Open a nearby person’s choices and select <strong>Invite to hangout</strong>, or type <code>hangout invite Pat</code>. A nearby authored resident can join and contribute. Their contribution has an activity sound and goes into the same shared memory as everyone else’s. Inviting a human player leaves the choice to join with them. Invitations do not move people from other rooms.</p>
<p>If the host leaves and only authored residents remain, the gathering closes and saves its memories. Otherwise hosting passes to another player. Repeating a resident invitation does not add another copy of them or their contribution.</p>
...
<p>Walking now follows the ground: boards, hard floors, carpet, grass, earth, metal, and wet ground have different cues. Rain turns soft paths muddy. The web world adds woodland, creek, campfire, home, and library backgrounds, with quieter outdoor sound indoors. The ten ward backgrounds now have corrected recording levels. Reopen the World to pick them up; your sound switches still apply. On the website, rooms without a separate room tone keep more of their neighborhood sound, and your next tap can restart a paused background. Background sound pauses when the web page is hidden. Choose <strong>Notice surroundings</strong> for the sounds, smells, and feel of the place in words. Pictures can be hidden in settings.</p>
...
<p>Ordinary city routines run without a model. A small Founder-funded trial lets residents choose a few quiet activities while someone is nearby. It stops after its fixed allowance or trial window; ordinary routines continue. It cannot make purchases, move people, or change relationships. Optional resident conversations have their own fixed AI allowance. Neither allowance renews automatically. The dollars in your wallet are game money. No payment is taken from players for these activities.</p>
...
<p><strong>Sources and corrections:</strong> open this link on a memory card to inspect its recorded conversations and correct selected related cards. Older memories without source evidence say so. To exclude a saved chat from future remembering, open <strong>Memory controls</strong> in its web chat header. This blocks new memory writes from that conversation and recall of cards, diary entries and summaries with that recorded source. The conversation remains saved. Untraceable older memories need to be reviewed separately; the control cannot infer missing history.</p>
...
<h2>Good to know</h2>
<ul>
  <li>Each Seed Audio clip is up to about <strong>2 minutes</strong> long. Want something longer? Ask for it in parts and Cadence can stitch them together &mdash; or use a longer <em>narration</em> (below) within the Booth's script limits.</li>
  <li>Seed Audio speaks <strong>twenty languages</strong>; ask Cadence to name them if you need one in particular.</li>
</ul>

<h2>Stable Audio: sound effects and ambience</h2>
<p>Choose <strong>Stable Audio</strong> in the <a href="/sound-booth">Sound Booth</a> for environmental sound, foley and layered ambience. Open <strong>Sound settings</strong> and use <strong>Sound model</strong> to choose <strong>3 Medium</strong>, the new default, or <strong>3 Small SFX</strong>, the original option. Medium is the larger model and our recommended starting point for detailed ambience. Both run through fal and save stereo WAV files; width and layer accuracy vary between recordings. Describe the foreground sound, quieter background layers and the space around them. Try the rain-cabin, harbor or forest-stream starting points. Ask for no music or speech when you want only environmental sound. This produces one mixed recording, not separate stems.</p>
<p>Enter an optional <strong>Track title</strong>, choose <strong>1 to 120 seconds</strong> and <strong>1 to 4 takes</strong>, then press <strong>Generate sounds</strong> once. Each take uses a different seed. All takes are submitted together; fal controls how many run at once. Provider cost is <strong>3.76 cents per Medium recording</strong>, or <strong>15.04 cents for four</strong>. Small SFX remains <strong>2.06 cents per recording</strong>, or <strong>8.24 cents for four</strong>. Prices verified September 18, 2026. During this trial, Kade pays that cost and nothing is deducted from your credit balance. The price appears before you generate; there is no additional confirmation screen. Reopening a saved project restores the model used for it, including older Small SFX projects.</p>
<p>Finished recordings are kept as <strong>lossless WAV</strong> in your Sound Booth library and My Creations. You can leave the screen while a batch runs. One completion notification opens the Sound Booth, subject to your notification settings. Finished takes survive another take failing; Stop requests cancellation but a take already running may still finish and incur a provider charge. Rename tracks in the library, or reopen a description to make another batch.</p>
<p><strong>Inference steps</strong> default to 8 for both distilled models; increasing them takes longer and does not guarantee better sound. An optional seed lets you reuse a starting point. This SFX engine does not offer voice cloning, cover-song controls or a guidance slider. Seed Audio remains available for scenes with dialogue and layered soundscapes. Reopen Sound Booth to load the new model picker; the existing iPhone build supports it.</p>

<h2>AuK HQ: speech and recording edits</h2>
<p>On the website and iPhone build 300 or later, start in the main writing box. <strong>Write a script from this</strong> turns your idea into editable text using the writing model. For music, the button shapes your music idea; YuE2 can draft the lyrics too. <strong>Surprise me</strong> gives you a starting idea: for songs the writer gives you one idea the way a songwriter would jot it down, a genre and one very specific human situation, sometimes with a rule such as never say the word sorry; it takes about ten seconds, costs a fraction of a cent, and remembers what it has already shown you so it does not repeat itself, and for speech and scenes it rolls a free one on the page. If the writer cannot be reached, you get a free idea from the short list instead; <strong>Undo writing change</strong> restores what was there. None of these buttons starts an audio recording. Voice, references and detailed settings remain in expandable sections. The separate writing desk still offers exact-word formatting.</p>
<p><strong>YuE2</strong> is a separate music option alongside Lyria. Describe the style and add Lyrics in song settings, or use <strong>Write my song idea</strong>. Its full-quality model runs on sleeping RunPod GPUs. YuE2 can use up to two GPUs at once; AuK keeps its own worker. Each GPU stays awake for ten minutes after its last job, then scales to zero. Startup, generation and awake idle time are billed, at about $1.22 per GPU hour on the selected cards. Ten idle minutes is about twenty cents per GPU; there is no monthly GPU reservation or new persistent storage rental. The library's YuE2 cost is an execution estimate; startup and idle add to Kade's provider bill. YuE2 currently deducts nothing from user credit balances. Lyria records and deducts its per-song charge from existing non-admin balances; admin accounts are exempt. A requested song length is guidance, not an exact duration.</p>
<p>To make a <strong>YuE2 cover</strong>, import a source recording in song settings, describe the new style, and add the words to sing in Lyrics. Or choose <strong>Cover this take</strong> on a saved Lyria or YuE2 song. The original stays in your library. Recordings can be WAV, MP3, M4A or OGG, up to twenty megabytes and six minutes. The worker transcribes the melody and then makes a new arrangement on the same GPU. Melody transcription can make mistakes and does not clone the singer. After importing, choose <strong>Transcribe reference lyrics</strong> to have Gemini listen to the recording and draft the sung words in lyric lines, with ElevenLabs Scribe as a fallback. It preserves repeated choruses and does not use your saved lyrics to fill gaps. Correct wrong or missing words in Lyrics and add verse or chorus tags as needed. <strong>Undo writing change</strong> restores the previous lyrics. This button does not generate music or deduct credits; completed transcripts from the current transcriber are reused. Older Deepgram drafts are replaced the next time you press Transcribe reference lyrics. Your saved writing changes only when you choose that button. You can also supply an ABC melody score instead of a recording. Voice cloning, LoRAs and saved singer personas are not offered by this integration. A take that reaches a model limit is marked as possibly ending early. The website also offers a composition-score download.</p>
<p><strong>Sound Booth lyric drafts now follow Lyric:</strong> the music writing desk reads the Lyric agent's current saved persona for each new draft. Its craft guidance covers connected thoughts, believable voices, internal and multisyllabic rhyme when appropriate, and a private edit for forced rhymes and stock phrasing. The desk now writes with Kimi K3 and always thinks the song through before writing, even for a one-sentence idea. It now writes by Kade's own hit-writing system: one moment, one voice with an attitude, the hook built first, open vowels under the big notes, a planted detail that pays off, and variety where a machine would be uniform. Unless you ask for another length it writes a song of about four minutes with three verses. The writer now takes its time: a song draft runs in the background for about five minutes while the writer thinks the song through, then goes back over it once like a producer deciding whether the song gets cut. You do not have to wait on the page. You press Help write this once. Stay and the draft appears in the editor by itself; leave, and a notice arrives when it is ready, and the Sound Booth page picks the draft up on its own the next time you open it. If you are in a hurry, tick Quick song draft on the website for a draft in about a minute and a half, less polished. iPhone builds before 303 still use the quick draft. It avoids the words and props that make a lyric sound machine-made, such as shadows, whispers and neon, but also the everyday ones: naming a weekday, coffee, the porch light, the kitchen table, two a.m. After writing, the desk checks the draft for those and has the writer replace just those lines, keeping everything else. If you want one of those words, put it in your idea and it is yours. To ask for a shorter song, say so in your idea. It keeps the Sound Booth output format. If the writer cannot finish, your idea is kept and you can try again. Existing lyrics supplied to the desk stay unchanged. To replace a draft you dislike, clear the Lyrics box before choosing Write my song idea. Drafting new lyrics does not generate a recording. Prompt guidance improves the target, but cannot guarantee every line will land.</p>
<p>The Sound Booth script and music-brief writers, Lyric, Cole, and the character-draft writer share writing guidance to reduce canned AI phrasing. It asks for concrete detail, natural rhythm and a private revision check, while protecting deliberate choruses, rhyme, dialogue and required formatting. Formatting-only requests retain your supplied words. This improves consistency but does not guarantee every draft will be free of cliches. Writing a draft does not automatically order a recording.</p>
<p>Sound Booth uses <strong>Kimi K3</strong> from Moonshot AI, reached through OpenRouter, for new music and lyric drafts. It was chosen by comparing drafts from several writers on the same song ideas, and it writes explicit lyrics when you ask for them. A draft costs roughly 2 to 3 cents in provider charges, up from about 1 cent. Speech and scene writing, plus exact-word formatting, use <strong>Hermes 4 405B</strong>. Lyric, Cole and the character builder keep their own models. Sound Booth records the provider's writing cost when available, otherwise a token-based estimate. This choice applies to the text writer; AuK, Seed Audio and Lyria still make the recordings with their own capabilities and provider rules. Your existing drafts are not automatically rewritten.</p>
<p>The Sound Booth uses <strong>AuK Base</strong> for speech and editing, <strong>Seed Audio</strong> for scenes and backgrounds, and <strong>Lyria</strong> for songs. Open Tools, Sound Booth in the native app, or <a href="/sound-booth">the web Sound Booth</a>. Each engine retains its own draft.</p>
<p>For speech without a reference clip, describe the voice's accent, texture, mood and delivery, then write only the words to say in the performance script. Later sections reuse the opening voice. An imported reference supplies its own voice and accent; changing the description does not change that reference. The voice wheel supplies a description when no clip is attached; it does not copy the wheel's original speaker.</p>
<p>To try giving a reference voice another accent, choose <strong>Edit</strong>, import the original recording, and describe the accent change while asking to preserve the words and speaker identity. Listen to the new take, then choose <strong>Use this voice</strong> for your script. Adding an arbitrary accent is experimental; AuK documents removing a regional accent. The booth does not automatically run or charge for a separate accent-edit pass.</p>
<h3>Edit an existing recording</h3>
<ol>
<li>Choose <strong>edit</strong> under Task and import the recording.</li>
<li>In <strong>Edit instructions</strong>, describe the change: make the delivery cheerful, change a word, whisper, raise pitch, remove noise or reverb, or separate a voice from the background. Say what should stay the same.</li>
<li>Leave <strong>Target seconds</strong> blank for the original duration, or set it when changing speed or adding or removing words.</li>
<li>Read the cost information, then choose Edit recording. On the website AuK starts with one press. The result is a new take; your source and previous takes remain available.</li>
</ol>
<p>Long work is processed in sections and joined. Listen back for seams, missed word edits and voice changes. Accent instructions are requests, not a guarantee. AuK does not automatically retry pronunciation errors. Each take retains a 24 kHz mono WAV master and a listening MP3.</p>
<h3>Sleep and cost</h3>
<p>The rented GPU sleeps between jobs. Startup, processing and brief idle time are billable; availability and cold starts can delay a job. AuK currently has no reliable total price estimate. Its GPU rate is up to $1.22 per hour of active GPU time, not per hour of finished audio. Progress and Stop remain available. If the connection drops, reopen the existing project before starting another take. Stopping or failure does not prove that no GPU time was charged.</p>
<p>Generation now starts with one press on the website and in the updated iPhone app. The cost or no-estimate notice remains beside Generate. There is no extra render confirmation. Wait for imports or lyric transcription to finish first. If an import fails, retry it or choose <strong>Discard failed import</strong>. Completing an upload or transcription does not generate music automatically.</p>
<p><strong>Title and compare your music:</strong> enter an optional <strong>Track title</strong> before generating, or rename saved work in the library. YuE2 offers <strong>Number of takes</strong> from one to four. Takes use different seeds; up to two can generate in parallel when GPUs are available. You may leave the screen while they run. One notification opens the Sound Booth after the whole batch finishes, and finished takes remain available if another take fails. Quiet hours and notification mutes still apply.</p>
<p><strong>YuE2 advanced sound settings:</strong> Creative variation, or weirdness, changes the model's sampling temperature. Its default of 50 preserves the original settings; higher values explore less likely choices and can reduce coherence. Prompt guidance defaults to 1 and can increase to 3; higher values emphasize the conditions but add work and can reduce naturalness. Inference steps range from 16 to 64, with the original default of 32. More steps refine the audio longer but do not guarantee a better song. These controls are specific to YuE2. For an exact starting point, keep the seed, lyrics, direction and settings; a batch advances the seed for each take.</p>
<p>Failed attempts are visible by default in the web library, with their saved error. A render that fails while the booth is open shows a <strong>Generation stopped</strong> notice. Completion and failure pushes are requested results, so they do not use the ordinary agent-message allowance. Notification mutes still apply, and quiet-hour results wait for morning.</p>
<p>Use <strong>Seed Audio</strong> for newly generated music, sound effects and background scenes. AuK can clean or separate an existing background.</p>
<h3>Lyria: a song from a brief, written the way Google reads it (September 11, 2026)</h3>
<p>Lyria is Google's music engine, about <strong>eight cents a song</strong> however long it comes out, usually back in under a minute. It does not read speech and it cannot copy a particular voice; it makes a record. The booth's script desk and its how-to-write notes now follow Google's own prompt guide, in this order: the <strong>genre with an era</strong> ("1970s Memphis soul"), the <strong>instruments</strong> and what each one does, the <strong>shape</strong> as section tags ([Intro] &rarr; [Verse 1] &rarr; [Chorus] &hellip;) or timestamps, the <strong>voice</strong> if anybody sings (sex, timbre, range, delivery), the <strong>mood</strong> in a few plain adjectives, and last a <strong>technical line</strong> with a BPM number, the key, and how long ("around 70 BPM, in D minor, a two-minute song"). It reads the length from your words, so always say how long.</p>
<p>Your own words go in the <em>Your own lyrics</em> box, and the booth sends them under a <em>Lyrics:</em> heading the way the engine expects &mdash; put [Verse 1], [Chorus] and [Bridge] on their own lines above each part if you want to steer the shape. Turn on <em>No singing</em> for an instrumental. The words it writes come back cleaned of its own machine markers, so the read-back your screen reader hears is the song and nothing else; the raw text is kept with the recording in My Creations.</p>
...
<p>When you register yourself, you receive an email to verify your address. Open that link to finish signing up. When Kade approves an access request and creates your account, the approval email contains your sign-in information and temporary password instead. Change that password after signing in. If you already have an account, your password stays the same and the email helps you sign in. A successful password change also sends a confirmation email.</p>
...
PAGES.privacy = {
  ...
  main: `
<p class="lead">Kade-AI is a small, invite-only place run by Kade Murdock. Usage charges include a contribution toward running the platform, as explained in <a href="/help/donate">Usage &amp; Balance</a>. Your personal data is not sold, rented, or handed to advertisers &mdash; there are no ads on this platform and no advertising or tracking code in it. This page explains, honestly, what is kept and why.</p>
...
<ul>
  ...
  <li><strong>DeepSeek</strong> &mdash; since September 19, 2026 the DeepSeek V4.1 Flash model runs the characters, including Kiana and Della, as a trial. It is an open model run by many hosting companies, some outside the United States. Kade-AI only allows hosts that OpenRouter lists as zero data retention, meaning they keep no copy of your message after answering, and picks the cheapest of those. The host that answers receives your message and conversation context. An older DeepSeek model also names your conversations and helps read images, under the same rule.</li>
...
<p>Choose <strong>Export</strong> beside a file, <strong>Export this saved version</strong> while reading history, or <strong>Export this draft</strong> in the editor. Android asks where to save it. Export writes exactly the selected text as UTF-8, including line breaks, and does not save the project. The selected text is held while the picker is open, so a later project edit cannot change that export. You can save an unsaved draft locally before leaving the app.</p>
<p>Reading or writing a file runs off the main screen thread, has a 30-second deadline, and offers <strong>Cancel file transfer</strong>. An interrupted or failed export may leave a partial destination file: check it before explicitly trying again. Nothing is retried automatically. A picker cancellation leaves your draft unchanged. Word/PDF conversion and general chat attachments remain unavailable in this project flow.</p>
...
<p><strong>Manage project chats on the web</strong> opens the existing website for chat organization. Project rename/delete and chat assignment remain on the web. Version 2.8 adds native text-file import and export as described above. The native editor accepts typed or pasted text in .txt, .md, .csv and .json files, not Word or PDF documents.</p>
...
<p>ChatGPT export uploads check the file before sending and show progress. <strong>Cancel upload</strong> stops the upload connection; the server may already have stored conversations or started logbook work. <strong>Retry selected zip</strong> reuses the chosen file after an upload failure, and previously stored conversations are skipped. If file access has expired, pick the zip again. A confirmed import is not automatically repeated when a later memory or logbook step has trouble.</p>
...
<p><strong>More</strong> now includes <strong>Agent work</strong>, <strong>Projects</strong>, and <strong>What&rsquo;s new</strong>. Projects and What&rsquo;s new open signed-in platform pages; Agent work uses the native screen in version 2.5. When a page asks you for a file, Android&rsquo;s file picker opens; select the file there, then use the page&rsquo;s own controls to submit it.</p>
...
<p>In Android 2.7 (build 108), open <strong>More &rarr; Projects</strong> for native project search, creation, instruction/file editing and read-only version history. Reference files are maintained by you; working documents may also be revised by an agent in the project's chats. Text may be typed or pasted. Android 2.8 adds native UTF-8 text-file import and export. Chat management and project rename/delete remain on the website.</p>
...
<h2>Import and export text on Android</h2>
<p>Android 2.8 (build 109): choose <strong>Import a text file</strong> to open a .txt, .md, .csv or .json file as a new unsaved draft. Files must be UTF-8 text, at most 50,000 characters. Review the name, contents and Reference or Working document choice before <strong>Save a new version</strong>. The existing project limits still apply, including 60,000 total context characters and 30 files.</p>
<p>Export a current file from its card, an older version from its read-only viewer, or unsaved text with <strong>Export this draft</strong>. Android's picker selects the destination. Export preserves the selected text and line breaks as UTF-8 and does not change the server's project. Cancel or a 30-second timeout stops the transfer; failed exports may leave a partial file. Check the destination before trying again. There is no automatic retry.</p>
...
<p>Her searches use catalog facts, available source descriptions and existing scene notes. She can read passages from text books. Recordings without descriptions or transcripts may need more clues; a possible match is not a verified identification. She searches with your account's access: the family's shared collection and your own uploads. She cannot change or delete your collection.</p>
<p>Other characters can ask her too. When you ask Kiana or another character what the Library has, or about a book, tape, old broadcast or commercial, that character can check with Mrs. Witherspoon and pass her answer along. She searches with your access, and she cannot save requests or change anything when another character asks.</p>
<h2>Ask for something the library does not have</h2>
<p>Tell Mrs. Witherspoon what you are looking for: a book, audiobook, radio show, tape, commercial, TV show, movie, song or anything else. A title helps, but a few remembered details are enough. She searches first. If it is not here, she saves a library request and gives you its link. Requests are for family library members.</p>
<p>Kade reviews every request. A saved request is not a promise that something can be found. When Kade fills your request, or finds it cannot be filled, you hear about it: a phone notification if Kade-AI notifications are on, a browser notification if you allowed them, and a short note from the next character you talk to. The iPhone app does not have a requests screen yet; on the phone, ask Mrs. Witherspoon about your requests.</p>
<p>On the website, the <strong>Library requests</strong> section of the Library's main page lists your requests, with new updates marked. Open one to read its history, add details, or cancel it. Asking again for something you already requested adds to the same request instead of making a second one.</p>
...
<h2>Read along</h2>
<p>On a text book's player on the website, <strong>Open reading view</strong> shows the book's own words full screen, to read on your own or while it is narrated. Narration tags never appear on screen. Under <strong>Text size, colors and spacing</strong> you can choose text size up to Largest, colors including white on black and yellow on black, line spacing, letter and word spacing, line width, and a typeface such as Lexend or OpenDyslexic. Your choices are remembered for your account on that device, and the view starts from the contrast, font and spacing you chose in the chat's Settings.</p>
<p>A reading page holds about 18 minutes of narration. <strong>Play narration</strong> works inside the view. Previous page, Next page and the Chapter list move you on your own; the narration keeps its own place. <strong>Show where the narration is</strong> jumps to the passage being read, and <strong>Turn pages with the narration</strong> keeps the view with it while you stay at that passage. When the narration turns the page, focus moves to the new page's heading. Escape or <strong>Close reading view</strong> returns you to the player.</p>
<p>Accessible editions often begin with a distribution notice. The Library leaves those notices out of the reading view and the narration. The book's opening instead says it was produced for people with bona fide print disabilities.</p>
<h2>The new iPhone player</h2><p>In the new TestFlight update, Play or Pause stays at the bottom center, Back on its left and Forward on its right. A tap moves ten seconds. Hold either seek button for a menu of 10, 20, 30 seconds, one minute, or two minutes; VoiceOver also offers the larger jumps as actions. Chapter buttons remain separate above this row. Recorded audio and video have an adjustable position slider.</p>
<p>Text-book seeking uses the generated recording's time. Revisiting cached passages is quicker; jumping into a passage that has not been made yet can take a moment. Wait for the jump announcement before seeking again. Your place and new bookmarks include the position within the passage.</p>
...
<p>On the website, start with <strong>Springfield, the Ozarks &amp; Missouri</strong>, or browse commercials by subject and TV clips by show. Station names remain searchable. Other stations’ presentation lives under Broadcast Archives. Search offers <strong>More search results</strong> when there are over 100 matches.</p>
<h2>Add a file</h2><p>Share a file from another app to Kade-AI and choose the Library, or use the Add controls. DAISY text and recorded-audio ZIPs use the same book importer. Recorded DAISY plays its original narrator, in the publication’s order, with chapter navigation and saved progress. On the website, audiobook ZIPs may be up to 4 GB (8 GB expanded). ZIPs containing MP3 or M4B recordings stay together as one audiobook, in numbered file order. Text-book imports remain limited to 256 MB. The current iPhone book-upload picker still has a 256 MB limit; use the website for larger ZIPs. Encrypted audio and missing recordings cannot be imported. Text books use the voice you select.</p>
...
<h2>Private and public</h2><p>Your uploads belong to you. Regular members submit items for the librarian to approve; until then they are private. The librarian’s uploads are shared by default, with a Keep this upload private control. Uploads from a few trusted family members go into the family library without waiting. Sharing does not give other people your reading progress.</p>
<h2>Who can open the family library</h2><p>The shared collection is for family. Family accounts that existed before this change on September 24, 2026 keep access. A newer account can search and play only its own uploads until Kade turns on family library access for it, and Mrs. Witherspoon tells them so plainly instead of saying something is missing. Kade manages this in the <strong>Family library access</strong> section of the Library page, which only she sees.</p>
<h2>Filing corrections</h2><p>Use Suggest a different shelf on an item. The librarian can move it without uploading it again. TubeVault’s source paths stay attached for duplicate detection. Reviewed titles and clear product matches keep toy ads, medicines, household products and recordings on the right shelves. Familiar shows are grouped by show, with their source network retained. Custom filing rules stay yours; ambiguous titles stay for review. Inspirational story collections have their own nonfiction shelf.</p>
<h2>Donate from a Windows computer</h2><p><a href="/help/library-uploader-download">Download Kade Library Uploader for Windows (EXE, 17 MB)</a> · <a href="/help/library-uploader-zip">Download the app and guide together (ZIP, 17 MB)</a></p><p>Version 1.4 for Windows 10 or 11, 64-bit. Unpacked DAISY books need no manual zipping: choose Add folder and select the SD-card folder containing your books, or a single book folder. Each recognized book becomes one queue item with its text, navigation, pictures, and audio kept together. Review titles in Edit details. The app creates one temporary ZIP at a time on your PC and leaves the SD card unchanged. Keep the card connected until uploads finish and allow free space for the largest book. Audiobook MP3/M4B and DAISY ZIPs can be up to 4 GB; text-book files remain limited to 256 MB. Books, audio and video upload directly to storage. Book processing continues separately after the file arrives. If interrupted, choose Start / resume with the same file to check the same import. Older Check library entries still need a manual check before retrying. Finish or pause current uploads and close the older uploader before opening this version; your saved queue is retained. Uploads and Your library now use standard Windows list boxes with complete spoken row labels. The first row is selected automatically, refreshes keep your selection, and Your library loads when its tab opens. Open the EXE and sign in with your own platform account. Choose Donate for review or Keep private, add files or a folder, check Edit details, then choose Start / resume. It keeps Books, Audio, and Videos separate and lets you organize your own uploaded items. Ordinary-member donations remain private until reviewed; an administrator can publish directly.</p>
<h2>MP3 audiobook ZIPs in TubeVault</h2><p>TubeVault's updated library helper, version 1.8, sends the MP3 or M4B chapters directly to storage and keeps them together as one audiobook in numbered order. Choose the audiobook ZIP folder with Books enabled, then Send. Progress names each chapter. Send again after an interruption to resume the same audiobook; completed chapters are retained. Keep the original ZIP and the helper's resume folder unchanged until finished. The original files stay on your computer. Sharing happens after all chapters arrive.</p><p>This avoids the website's five-minute incoming-transfer limit that can appear as an upstream error on large ZIPs. DAISY and other supported books now also upload directly to storage, then finish importing separately. Windows uploader 1.4 and iPhone TestFlight 298 use direct storage too. Website and Android Library uploads currently need a storage configuration repair; use the Windows uploader or updated iPhone app while that repair is pending. Audio and video also bypass the website for file transfers; large recordings use multipart uploads. Other connection failures can still occur, but these updated paths avoid Railway’s incoming-body timeout. The TubeVault executable version and library-helper version are separate; an existing upload run keeps its loaded helper until the next Send.</p>
