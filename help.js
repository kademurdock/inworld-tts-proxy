...
<p>Kade-AI now runs a second audio engine, <strong>Scenema Audio</strong>, on a RunPod-hosted graphics card. Your text and reference clips are sent to that hosted worker. Where Seed Audio makes a whole scene, Scenema makes a <em>performance</em>: one voice reading your words with stage directions between the lines &mdash; the feeling can shift mid-sentence, it breathes, it pauses, it can crack on a word. Describe the actor in words ("a woman in her sixties, low and warm, a little tired, Southern") or hand it a ten-second clip of a voice to copy. It is built for bedtime stories, monologues, letters read aloud, and whole audiobook chapters: the Booth automatically divides supported longer scripts into parts.</p>
<ul>
  <li>Ask Cadence, or any character with the studio tool, for a <strong>narration</strong>. It is a queued job, not instant: check <strong>My Creations</strong> when it is ready; notifications depend on your settings. A cold start can add about seven minutes; if no graphics card is free, the queue can be longer. The progress line explains the wait. If the connection drops, ask the agent to check the existing narration before ordering another render. A failed or missing job does not prove that the provider charged nothing.</li>
  <li>It is billed for <strong>GPU time</strong>, including a wake allowance. Read the current estimate before confirming. There is a small daily and monthly budget for it; the character will say so if it is used up.</li>
...
<p>Everything above happens by <em>asking a character</em>. The <strong>Sound Booth</strong> is the same engines with your hands on them directly — it's on the home screen in the app, under Tools, and on the web at <a href="/sound-booth">/sound-booth</a>. Since September 10 it has a third engine, <strong>Lyria</strong>, that writes a whole song and sings it.</p>
...
<h3>Which engine? Ask one question.</h3>
<p>Is this <strong>one person talking</strong>, or a <strong>scene</strong>? One person reading a story, a letter, a monologue, a bedtime tale — with real acting — is <strong>Scenema</strong>. Two people talking, or anything with music, sound effects, or a place you can hear, is <strong>Seed Audio</strong>. The Booth has a "which one should I use?" note right under the engine cards, and a button that reads what you typed and picks for you, with the reason.</p>
<p>They are written differently, and the Booth shows you how for whichever you pick. For Scenema, describe <em>who</em> is speaking in one specific sentence — sex, age, accent, texture, manner — and between your sentences tell the actor what they are <em>doing and feeling</em> ("voice tightens, swallows"), never what the sound should be like. For Seed Audio, write a short scene brief: the place, who is in it, the music and sounds (spell them out — a door "slap", a bell "ring-a-ling"), and each line as <em>Name (voice) says, how: "the words."</em> The settings match the engine too: Scenema has scene, shot, pace and one clip to clone; Seed Audio has preset voices, speed, pitch, volume, and up to three clips. On Scenema you can <strong>hear the voice first</strong> — a short paid sample with its own estimate — before rendering the whole piece.</p>
<ul>
...
<h3>What actually changes a Scenema take, and what doesn't (September 4, 2026)</h3>
<p>There is <strong>no temperature</strong> and no "sample more" setting on this engine, so if a take sounds wrong, nothing you could have turned would have fixed it. Scenema runs a fixed eight steps; the only dice is the <strong>seed</strong>, and the same seed with the same script is the same take again. The Booth keeps one seed per project so your preview and your render are the same actor.</p>
<p>What does change a take, in order: <strong>the words in the voice description</strong> (this is the main control &mdash; sex, age, texture, accent, manner, one line of character); <strong>the stage directions</strong> between sentences, written as what the speaker is doing and feeling, never what the sound should be like; <strong>the clip</strong>, if you attach one &mdash; ten to twenty seconds of one person, clean, with some feeling in it, because a flat or noisy clip clones thin (the Booth now converts every clip to a studio WAV on import and tells you its length); <strong>Shot</strong> and <strong>Scene sound</strong> for anything you want to hear around the voice; and <strong>Pace</strong>. A cloned voice can flatten the acting &mdash; if it does, put a stronger character into the description, or set <em>How much of the clip</em> to Natural.</p>
<p>Under the hood the engine listens back to every fifteen-second piece and re-makes it up to three times if the words came out garbled; that is the <em>Pronunciation check</em>, on by default. Hard names still get mangled sometimes &mdash; spell them the way they sound.</p>
