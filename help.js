...
<p><strong>YuE2</strong> is a separate music option alongside Lyria. Describe the style and add Lyrics in song settings, or use <strong>Write my song idea</strong>. Its full-quality model runs on a separate sleeping RunPod worker. AuK and YuE2 each stay awake for ten minutes after their last job, then scale to zero. Startup, generation and awake idle time are billed, at about $1.22 per GPU hour on the selected cards. Ten idle minutes is about twenty cents per service; there is no monthly GPU reservation or new persistent storage rental. The library's YuE2 cost is an execution estimate; startup and idle add to the provider bill. A requested song length is guidance, not an exact duration.</p>
<p>To make a <strong>YuE2 cover</strong>, import a source recording in song settings, describe the new style, and add the words to sing in Lyrics. Or choose <strong>Cover this take</strong> on a saved Lyria or YuE2 song. The original stays in your library. Recordings can be WAV, MP3, M4A or OGG, up to twenty megabytes and six minutes. The worker transcribes the melody and then makes a new arrangement on the same GPU. Transcription can make mistakes; it does not extract lyrics or clone the singer. You can also supply an ABC melody score instead of a recording. Voice cloning, LoRAs and saved singer personas are not offered by this integration. A take that reaches a model limit is marked as possibly ending early. The website also offers a composition-score download.</p>
...
<p>On the website, AuK performance, preview and editing start paid generation with one press. The GPU cost notice remains beside the controls; the final cost is available afterward. Generation waits for reference imports to finish. If an import fails, retry it or choose <strong>Discard failed import</strong> before generating. Completing an upload does not start generation automatically. Seed Audio and Lyria retain their price confirmation, with the price attached to the focused <strong>Start paid generation</strong> button for screen readers. iPhone build299 still uses the confirmation flow; the matching native changes are awaiting a new build.</p>
