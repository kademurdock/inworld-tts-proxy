...
<ul>
  <li><strong>The iPhone voice picker caught up (build 262 in TestFlight).</strong> Same shape as the site now: a <strong>Category</strong> row first (all twenty-three, with a count on each, and "All voices" at the bottom), then that category's voices. It opens on whichever category holds your current voice, an old pick like "Voice 69" keeps its checkmark on the label it became, VoiceOver reads a one-sentence description of each voice as the row's hint, and the search box understands those words too. Previews are still on the rotor (full audition, quick preview, hear another sample) &mdash; nothing plays until you ask.</li>
